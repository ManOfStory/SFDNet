...
from .rotated_bbox_head import RotatedBBoxHead, RotatedUncBBoxHead
...
@ROTATED_HEADS.register_module()
class RotatedConvFCUncBBoxHead(RotatedUncBBoxHead):
    r"""More general bbox head, with shared conv and fc layers and two optional
    separated branches.
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(RotatedConvFCUncBBoxHead, self).__init__(
            *args, init_cfg=init_cfg, **kwargs)
        assert (num_shared_convs + num_shared_fcs + num_cls_convs +
                num_cls_fcs + num_reg_convs + num_reg_fcs > 0)
        if num_cls_convs > 0 or num_reg_convs > 0:
...
@ROTATED_HEADS.register_module()
class RotatedShared2FCUncBBoxHead(RotatedConvFCUncBBoxHead):
    """Shared2FC RBBox head."""

    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(RotatedShared2FCUncBBoxHead, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
