...
from mmrotate.core import build_bbox_coder, multiclass_nms_rotated, multiclass_unc_nms_rotated
...
@ROTATED_HEADS.register_module()
class RotatedUncBBoxHead(BaseModule):
    """Simplest RoI head, with only two fc layers for classification and
    regression respectively.
    ...
    def __init__(self,
                 ...
                 loss_bbox=dict(
                     type='SmoothL1Loss', beta=1.0, loss_weight=1.0),
                 init_cfg=None):
        super(RotatedUncBBoxHead, self).__init__(init_cfg)
        assert with_cls or with_reg
        self.with_avg_pool = with_avg_pool
        self.with_cls = with_cls
        self.with_reg = with_reg
            ...
            if pos_inds.any():
                ...
                if self.reg_class_agnostic:
                    pos_bbox_pred_mu = bbox_pred_mu.view(
                        bbox_pred_mu.size(0), 5)[pos_inds.type(torch.bool)]
                    pos_bbox_pred_var = bbox_pred_var.view(
                        bbox_pred_var.size(0), 5)[pos_inds.type(torch.bool)]
                else:
                    pos_bbox_pred_mu = bbox_pred_mu.view(
    ...
    def get_bboxes(self,
        ...
        if cfg is None:
            ...
            det_bboxes, det_labels, det_bboxes_unc, inds = multiclass_unc_nms_rotated(
                bboxes, scores, bboxes_unc, cfg.score_thr, cfg.nms, cfg.max_per_img, return_inds=True)
