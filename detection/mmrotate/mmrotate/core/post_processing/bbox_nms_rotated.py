...
def multiclass_unc_nms_rotated(multi_bboxes,
                           multi_scores,
                           multi_uncertainties,
                           score_thr,
                           nms,
    ...
    if multi_uncertainties.shape[1] > 5:
        uncertainties = multi_uncertainties.view(multi_scores.size(0), -1, 5)
    else:
    ...
    labels = labels.view(1, -1).expand_as(scores)
    bboxes = bboxes.reshape(-1, 5)
    uncertainties = uncertainties.reshape(-1, 5)
    scores = scores.reshape(-1)
    labels = labels.reshape(-1)
    # remove low scoring boxes
    valid_mask = scores > score_thr
    ...
    inds = valid_mask.nonzero(as_tuple=False).squeeze(1)
    bboxes, scores, labels, uncertainties = bboxes[inds], scores[inds], labels[inds], uncertainties[inds]

    if bboxes.numel() == 0:
        dets = torch.cat([bboxes, scores[:, None]], -1)
        if return_inds:
            return dets, labels, uncertainties, inds
        else:
            return dets, labels, uncertainties
    ...
    if return_inds:
        return torch.cat([bboxes, scores[:, None]], 1), labels, uncertainties, inds[keep]
    else:
        return torch.cat([bboxes, scores[:, None]], 1), labels, uncertainties
