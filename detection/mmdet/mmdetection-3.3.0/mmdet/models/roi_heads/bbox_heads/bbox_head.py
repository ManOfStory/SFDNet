...
class BBoxHead(BaseModule):
    ...
    def _predict_by_feat_single(
        ...
        if rcnn_test_cfg is None:
            ...
            det_bboxes, det_labels, kept_inds = multiclass_nms(
                bboxes,
                scores,
            ...
            N, C, H, W = bbox_feats.shape
            bbox_feats = bbox_feats[:, None].repeat(1, num_classes, 1, 1, 1).reshape(-1, C, H, W)
            bbox_feats = bbox_feats[kept_inds]
            results.bbox_feats = bbox_feats
        return results
...
class BBoxUncHead(BaseModule):
    ...
    def loss(self,
        ...
        Args:
            cls_score (Tensor): Classification prediction
                results of all class, has shape
                (batch_size * num_proposals_single_image, num_classes)
            bbox_pred_mu (Tensor): Regression prediction mu results,
                has shape
                (batch_size * num_proposals_single_image, 4), the last
                dimension 4 represents [tl_x, tl_y, br_x, br_y].
            bbox_pred_var (Tensor): Regression prediction var results,
                has shape
                (batch_size * num_proposals_single_image, 4), the last
                dimension 4 represents [tl_x, tl_y, br_x, br_y].
            rois (Tensor): RoIs with the shape
        ...
        if bbox_pred_mu is not None and bbox_pred_var is not None:
            ...
            if pos_inds.any():
                if self.reg_decoded_bbox:
                    ...
                    bbox_pred_mu = get_box_tensor(bbox_pred_mu)
    ...
    def predict_by_feat(self,
                        rois: Tuple[Tensor],
                        cls_scores: Tuple[Tensor],
                        bbox_preds_mu: Tuple[Tensor],
                        bbox_preds_var: Tuple[Tensor],
                        bbox_feats: Tuple[Tensor],
                        batch_img_metas: List[dict],
                        rcnn_test_cfg: Optional[ConfigDict] = None,
                        rescale: bool = False) -> InstanceList:
        ...
        Returns:
            ...
            Each item usually contains following keys.
                ...
                - bboxes (Tensor): Has a shape (num_instances, 4),
                  the last dimension 4 arrange as (x1, y1, x2, y2).
        """
        assert len(cls_scores) == len(bbox_preds_mu)
        result_list = []
        for img_id in range(len(batch_img_metas)):
            img_meta = batch_img_metas[img_id]
            results = self._predict_by_feat_single(
                roi=rois[img_id],
                cls_score=cls_scores[img_id],
                bbox_pred_mu =bbox_preds_mu[img_id],
                bbox_pred_var = bbox_preds_var[img_id], 
                bbox_feats = bbox_feats[img_id],
                img_meta=img_meta,
                rescale=rescale,
                rcnn_test_cfg=rcnn_test_cfg)
    ...
    def _predict_by_feat_single(
        ...
        if bbox_pred_mu is not None and bbox_pred_var is not None:
            num_classes = 1 if self.reg_class_agnostic else self.num_classes
            roi = roi.repeat_interleave(num_classes, dim=0)
            bbox_pred_mu = bbox_pred_mu.view(-1, self.bbox_coder.encode_size)
            bboxes = self.bbox_coder.decode(
                roi[..., 1:], bbox_pred_mu, max_shape=img_shape)
            bboxes_unc = bbox_pred_var
        else:
            bboxes = roi[:, 1:].clone()
            bboxes_unc = roi[:, 1:].clone()
            if img_shape is not None and bboxes.size(-1) == 4:
                bboxes[:, [0, 2]].clamp_(min=0, max=img_shape[1])
                bboxes[:, [1, 3]].clamp_(min=0, max=img_shape[0])
    ...
    def refine_bboxes(self, sampling_results: Union[List[SamplingResult],
        ...
        cls_scores = bbox_results['cls_score']
        rois = bbox_results['rois']
        bbox_preds = bbox_results['bbox_pred_mu']
        if self.custom_activation:
            # TODO: Create a SeasawBBoxHead to simplified logic in BBoxHead
            cls_scores = self.loss_cls.get_activation(cls_scores)
        if cls_scores.numel() == 0:
