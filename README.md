...
📌 **Note:** This branch contains the **HBB** version code. If you are looking for the **OBB (Oriented Bounding Box / Rotate)** version, please switch to the [`OBB`](../../tree/OBB) branch.
...
conda create -n SFDNet python=3.10 -y
conda activate SFDNet

# Upgrade pip and install PyTorch with CUDA 11.8 support
...
cd kernels/selective_scan && python setup.py install && cd ../..

# Compile Depth-wise Convolution kernel
cd kernels/dwconv2d && python setup.py install && cd ../..

# Install MMDetection & MMENGINE
cd detection/
cd mmdet/mmdetection-3.3.0 && python setup.py develop && cd ../..
cd mmengine && python setup.py develop && cd ..

# Install MMCV pre-built wheel
pip install mmcv-2.1.0-cp310-cp310-manylinux1_x86_64.whl

# Install AI-TOD COCOAPI Extension
cd kernels/cocoapi-aitod/aitodpycocotools && python setup.py install && cd ../../..
```
### 2. Dataset Preparation
The SODA dataset is processed following the protocol of CFINet. We provide the pre-processed datasets below:

- **AI-TOD**: [Quark Drive](https://pan.quark.cn/s/555ecad7497e?pwd=H682)
- **SODA-D**: [Quark Drive](https://pan.quark.cn/s/18ea7397429c?pwd=7tvJ)
...
```bash
# CNN version
python tools/train.py configs/SFDNet/aitod/SFDNet_CNN.py --work-dir SFDNet_CNN [--amp]

# Mamba version
python tools/train.py configs/SFDNet/aitod/SFDNet_Mamba.py --work-dir SFDNet_Mamba [--amp]
```

#### Evaluation
```bash
# CNN version
python tools/test.py configs/SFDNet/aitod/SFDNet_CNN.py SFDNet_CNN/epoch_xxx.pth
